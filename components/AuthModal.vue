	...
	if (login === 'phone') {
		...
		try {
			...
			if (data.value?.success) {
				console.log(data.value.success)
				authStore.smsError = ""
				authStore.emailButtonContent = "Код отправлен"
				authStore.smsStep = true
				const phoneNumber = data.value.tel
				if (phoneNumber) {
				...
				console.error(data.value?.error)
				authStore.smsError = data.value?.error ?? "Неизвестная ошибка"
				authStore.emailButtonContent = "Отправить СМС-код"
				authStore.smsStep = false
			}
...
const processPhoneNumber = async (phoneNumber: string): Promise<void> => {
	try {
		const availableCodes = [
			'+7', '+375', '+380', '+77', '+998', '+992', '+993', '+996', '+374', '+994', '+373', '+995'
		]
		let foundCode = ''
		let foundNumber = ''
		for (const code of availableCodes) {
			if (phoneNumber.startsWith(code)) {
				foundCode = code
				foundNumber = phoneNumber.slice(code.length)
				break
			}
		}
		if (foundCode && foundNumber) {
			...
			const country = countryMap[foundCode] || '';
			authStore.tel = {
				code: foundCode,
				phone: foundNumber,
				country: country,
			...
			console.warn('Не удалось разобрать телефонный номер:', phoneNumber)
		}
	} catch (error) {
		console.error('Ошибка при обработке телефонного номера:', error)
	}
}
...
const handleRegClick = async (): Promise<void> => {
	...
	try {
		...
		if (data.value?.success) {
			...
			console.error(data.value?.error)
			authStore.smsError = data.value?.error ?? "Неизвестная ошибка"
			authStore.regButtonContent = "Отправить СМС-код"
			authStore.smsStep = false
		}
      ...
      <div class="px-6 pb-6 flex flex-col items-stretch">
	      <template v-if="authStore.type === 'Авторизация'">
	        ...
	        <template v-else>
	          ...
	          <span v-if="remainingSeconds > 0" class="font-[Manrope] text-xs font-light text-[#211D1D] mt-8 self-center text-center">
	            Запросить код повторно <br >
	            можно через <span class="font-normal">{{ remainingSeconds }} секунд</span>
	          </span>
		        <span v-else class="font-[Manrope] text-xs font-light text-[#211D1D] mt-8 self-center text-center cursor-pointer" @click="handleResend">
			        Отправить ещё раз
		        </span>
	        </template>
	      </template>
