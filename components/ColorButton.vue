...
const emit = defineEmits<{
	(e: 'update:modelValue', value: { code: string, name: string, value: string } | { code: string, name: string, value: string }[] | null): void
}>()
...
<template>
    ...
    >
        ...
        >
            <div
	            :class="['w-6 h-6 rounded-lg', isSelected(color) && 'border-2 border-[#211D1D]']"
	            :style="{ backgroundColor: color.value }"
            />
        </button>
        <span v-if="text" class="text-xs font-[Manrope] text-center">{{ color.name }}</span>
    </div>
</template>
