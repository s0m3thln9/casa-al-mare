...
onMounted(() => {
	orderStore.setCartItems([{ id: '1', vector: 'red_xs-s', count: 1 }, { id: '2', vector: 'blue_xs-s', count: 2 }])
})
      ...
      <div class="p-8 w-full max-w-[564px] h-fit rounded-lg border-[0.7px] border-[#BBB8B6]">
            ...
            <div class="flex flex-col gap-6">
              <div v-for="(item, index) in orderStore.cartDetailed" :key="index" class="flex items-center justify-between w-full">
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <span class="text-xs text-[#414141]">{{ orderStore.priceFormatter(item!.price) }} <span class="font-light text-[#606060]">за шт.</span></span>
                  </div>
                </div>
                <div class="flex flex-col items-end gap-4">
                  ...
                  <span class="text-xs font-light">{{ orderStore.priceFormatter(item!.price * item!.count) }} <span v-if="item!.oldPrice > 0" class="line-through">{{ orderStore.priceFormatter(item!.oldPrice * item!.count) }}</span></span>
                </div>
              </div>
              <div class="flex flex-col gap-4">
                <div class="flex items-center justify-between cursor-pointer" @click="orderStore.togglePromoCode">
                  <span class="font-light text-sm">Промокод</span>
                  <button class="w-4 h-4 flex items-center justify-center cursor-pointer" :class="orderStore.isExpandedPromoCode ? 'rotate-0' : 'rotate-180'">
                    <NuxtImg src="/order-arrow.svg" class="w-full"/>
                  </button>
                </div>
                <div class="collapsible-div flex flex-col gap-4 transition-max-height duration-300 ease-in-out overflow-hidden" :class="{ 'max-h-500 opacity-100': orderStore.isExpandedPromoCode, 'max-h-0 opacity-0': !orderStore.isExpandedPromoCode }">
                  <AppInput
	                  id="promoCode"
	                  ...
	                  type="text"
                  />
                  <AppButton v-if="orderStore.promoCode" variant="primary" custom-class="w-full" content="Использовать" @click="orderStore.addPromoCode" />
                  <span v-if="orderStore.addPromoCodeError" class="font-light text-[13px] text-[#E57979]">{{ orderStore.addPromoCodeError }}</span>
                  <template v-for="promoCode in orderStore.currentPromoCodes" :key="promoCode.code">
                    <AppCheckbox v-model="orderStore.pendingPromoCode" size="S" :label="`Промокод “${promoCode.code}” -${promoCode.discount}% Выгода ${orderStore.savedMoney(promoCode.discount)} ₽`" :value="promoCode.code" />
                  </template>
                  <AppButton variant="primary" custom-class="w-full" content="Применить промокод" @click="orderStore.applyPromoCode" />
                  <span v-if="orderStore.usePromoCodeError" class="font-light text-[13px] text-[#E57979]">{{ orderStore.usePromoCodeError }}</span>
                </div>
              </div>
              <div v-if="authStore.isAuth" class="flex flex-col gap-4">
                <div class="flex items-center justify-between cursor-pointer" @click="orderStore.togglePoints">
                  <span class="font-light text-sm">Баллы</span>
                  <button class="w-4 h-4 flex items-center justify-center cursor-pointer" :class="orderStore.isExpandedPoints ? 'rotate-0' : 'rotate-180'">
                    <NuxtImg src="/order-arrow.svg" class="w-full"/>
                  </button>
                </div>
                <div class="collapsible-div flex flex-col gap-4 transition-max-height duration-300 ease-in-out overflow-hidden" :class="{ 'max-h-500 opacity-100': orderStore.isExpandedPoints, 'max-h-0 opacity-0': !orderStore.isExpandedPoints }">
                  ...
                  <AppInput
	                  ...
	                  type="text"
                  />
                  <AppButton variant="primary" custom-class="w-full" :content="`Списать ${orderStore.pendingPoints || 0} рублей`" @click="orderStore.applyPoints" />
                  <span v-if="orderStore.pointsError" class="font-light text-[13px] text-[#E57979]">{{ orderStore.pointsError }}</span>
                </div>
              </div>
              <div v-if="authStore.isAuth" class="flex flex-col gap-4">
                <div class="flex items-center justify-between cursor-pointer" @click="orderStore.toggleCert">
                  <span class="font-light text-sm">Сертификат</span>
                  <button class="w-4 h-4 flex items-center justify-center cursor-pointer" :class="orderStore.isExpandedCert ? 'rotate-0' : 'rotate-180'">
                    <NuxtImg src="/order-arrow.svg" class="w-full"/>
                  </button>
                </div>
                <div class="collapsible-div flex flex-col gap-4 transition-max-height duration-300 ease-in-out overflow-hidden" :class="{ 'max-h-500 opacity-100': orderStore.isExpandedCert, 'max-h-0 opacity-0': !orderStore.isExpandedCert }">
                  <AppInput
	                  ...
	                  type="text"
                  />
                  <AppButton v-if="orderStore.newCertificateCode" variant="primary" custom-class="w-full" content="Добавить сертификат" @click="orderStore.addCertificate" />
                  <span v-if="orderStore.certificateError" class="font-light text-[13px] text-[#E57979]">{{ orderStore.certificateError }}</span>
                  <template v-for="cert in userStore.user.certificates" :key="cert.code">
                    <AppCheckbox
              ...
              <div class="flex flex-col gap-1 text-sm font-light">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="flex items-center gap-2">{{ orderStore.priceFormatter(orderStore.totalSum) }} <span v-if="orderStore.totalOldSum > orderStore.totalSum" class="font-extralight line-through">{{ orderStore.priceFormatter(orderStore.totalOldSum) }}</span></span>
                </div>
                <div class="flex items-center justify-between">
                  <span>Окончательная стоимость:</span>
                  <span class="flex items-center gap-2">{{ orderStore.priceFormatter(orderStore.finalPrice) }} <span v-if="orderStore.totalSum > orderStore.finalPrice" class="font-extralight line-through">{{ orderStore.priceFormatter(orderStore.totalSum) }}</span></span>
                </div>
              </div>
        ...
        <div v-else-if="orderStore.isPaymentSuccessful">
          ...
          <div class="flex items-center justify-between mt-8">
            <span>Стоимость товаров:</span>
            <span class="flex items-center gap-2">{{ orderStore.priceFormatter(orderStore.finalPrice) }} <span v-if="orderStore.finalPrice !== orderStore.totalOldSum" class="font-extralight line-through">{{ orderStore.priceFormatter(orderStore.totalOldSum) }}</span></span>
          </div>
        </div>
        <div v-else>
          <div class="flex flex-col gap-6">
