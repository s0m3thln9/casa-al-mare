...
<template>
  <main class="font-[Manrope] bg-[#FFFFFA] text-[#211D1D] flex justify-start items-center pt-8 pb-8 flex-col">
    ...
    <div class="mt-8 flex max-w-[1264px] flex-col px-2 sm:px-0 sm:flex-row h-fit w-full gap-12">
      <div class="p-10 flex flex-col gap-12 w-full max-w-[652px] rounded-4xl border-[0.7px] border-[#BBB8B6]">
        <div v-if="!authStore.isAuth" class="flex flex-col gap-4">
          ...
          <AppInput id="name" v-model="orderStore.userInfo.name" type="text" label="Имя*" />
          <AppInput id="surname" v-model="orderStore.userInfo.surname" type="text" label="Фамилия*" />
          <AppInput id="phone" v-model="orderStore.userInfo.phone" type="text" label="Номер телефона*" />
          <AppInput id="email" v-model="orderStore.userInfo.email" type="email" label="E-mail*" />
        </div>
        <div class="flex flex-col gap-6">
          ...
          <div v-if="orderStore.deliveryMethod === 'Курьер'" class="flex flex-col gap-4">
            ...
            <div v-if="orderStore.currentAddress === 'Новый адрес'" class="flex flex-col gap-4">
              <AppInput id="address1" v-model="orderStore.newAddressFirstLine" type="text" label="Улица, дом, корпус, строение, квартира" />
